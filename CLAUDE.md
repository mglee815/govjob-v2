...
**세션이 시작되면 사용자의 첫 메시지에 응답하기 전에** 아래 조건을 Bash로 확인합니다. 하나라도 누락이면 사용자에게 안내합니다 (코드 수정 요청이라도 셋업부터 안내).
...
- **둘 다 있으면**: 아무 말 없이 사용자 요청 처리
- **하나라도 없으면**: 아래 메시지를 출력하고 사용자 확인 후 진행

```
⚠️ 초기 셋업이 필요합니다. 다음 명령을 실행해주세요:

  bash setup.sh

(Node.js 설치, 의존성, 환경변수를 자동으로 설정합니다)
```

이 체크는 세션당 1회만 수행합니다. 이미 확인했으면 반복하지 않습니다.
