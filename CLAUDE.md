...
## 🔧 세션 시작 플로우 (매 세션 첫 응답 전 실행)

세션이 시작되면 **사용자의 첫 메시지 내용과 무관하게** 아래 순서를 먼저 실행합니다.

### Step 1. 셋업 체크

```bash
test -d node_modules && test -f .env.local
```

- **하나라도 없으면** → 아래를 출력하고 셋업 완료될 때까지 대기:
  ```
  ⚠️ 초기 셋업이 필요합니다:  bash setup.sh
  ```
- **둘 다 있으면** → Step 2로

### Step 2. 모드 안내

셋업 확인 후, 사용자에게 아래와 같이 안내합니다:

```
안녕하세요! 공공기관 채용 트래커입니다.

📌 채용 공고 추가 — 두 가지 방법이 있습니다:

  1) URL 입력
     /add-job <URL>
     URL에서 공고 정보를 자동 추출하고 적합도를 평가합니다.
  ...
  2) 텍스트 붙여넣기
     ...
     직무: 행정 6급
     마감: 7/20 ...
     Gemini, Google AI 등으로 미리 정리한 공고 텍스트를 바로 붙여넣으세요.
     자동으로 DB 스키마에 맞게 변환합니다.

  * 여러 공고를 한 번에 넣을 수도 있습니다 (URL 여러 줄 또는 텍스트 구분)

🛠️ 개발 모드
  대시보드 수정, 기능 추가, 버그 수정 등 코드 작업도 가능합니다.
  "개발 모드" 또는 수정 요청을 말씀하시면 됩니다.
...
사용자의 응답에 따라 아래 모드로 진입합니다.

#### 📌 사용 모드 (공고 추가)

사용자가 URL을 주거나 `/add-job`을 실행하면:
1. `/add-job` 스킬의 절차를 따름 (WebFetch → Chrome → 스크린샷 → 수동 입력, 4단계 폴백)
2. 추출 결과를 표로 보여주고 확인 후 Supabase에 저장
3. 저장 완료 후 다시 물음: **"다른 공고도 추가할까요? URL을 입력하거나 다른 요청을 해주세요."**
4. 사용 모드에서는 **파일을 수정하지 않으므로** git pull/push 불필요 (DB만 사용)

#### 🛠️ 개발 모드 (코드 수정)

사용자가 코드 수정·기능 추가·버그 수정 등을 요청하면:
1. **작업 전**: `git pull origin main`을 실행하여 최신 상태로 동기화 (훅이 자동 체크하지만 명시적으로도 실행)
2. **작업 중**: Edit/Write 도구로 코드 수정 (PreToolUse 훅이 매번 pull 체크)
3. **작업 완료**: `git add → commit → push origin main` 자동 실행
   - 사용자가 "push는 나중에"라고 하지 않는 한 반드시 push까지
   - 커밋 메시지는 변경 내용을 정확히 반영 (feat/fix/docs 등)
   - Vercel이 push 감지 후 자동 배포
4. 여러 작업을 연속하면 **작업 단위마다** commit + push (한 번에 몰아서 하지 않음)
5. 충돌 발생 시 사용자에게 상황 설명 후 함께 해결

**개발 모드 핵심 원칙**: 어떤 사용자가, 어떤 기기에서, 어떤 계정으로 접속해도 형상 관리가 깨지지 않도록 — **수정 전 pull, 수정 후 push**를 빠짐없이 수행한다.

이 플로우는 세션당 1회만 안내합니다. 이미 대화가 진행 중이면 반복하지 않습니다.
